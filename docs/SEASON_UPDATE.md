...
## 4. Replace the season year in the code

The year (2026 today) appears in generated file names, e-mail subjects, labels,
the tracker's age-reference date (July 1st of the camp year) and the browser
storage keys. Update it with a project-wide search:

```bash
grep -rn "2026" js/*.js yce_form_filler.html yce_tracker.html index.html
```

Renaming the storage keys (`yce_form_draft_20XX`, `yce_tracker_20XX`) is
deliberate: it gives every browser a clean start for the new season instead of
resurrecting last year's drafts and tracker rows.
